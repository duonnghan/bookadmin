...
if(isset($_POST["action"]))
{
	...
	if($_POST["action"] == "insert")
	{
		...
		$query = "INSERT INTO book(bookname, price, description, cover, quantity, categoryid, publisherid, authorid) VALUES ('$name', '$price', '$description', '$file','$quantity', '$category', '$publisher','$author')";
	...
	{
		$file = addslashes(file_get_contents($_FILES["image"]["tmp_name"]));
		$query = "UPDATE test SET name = '$file' WHERE id = '".$_POST["image_id"]."'";
		if(mysqli_query($connect, $query))
		{
	...
	{
		$query = "DELETE FROM test WHERE id = '".$_POST["image_id"]."'";
		if(mysqli_query($connect, $query))
		{
